...
static void SdlCreateWindow(ecs_iter_t *it) {
  ...
  ecs_entity_t ecs_id(AppWindowHandle) = ecs_term_id(it, 2);

  for (int i = 0; i < it->count; i++) {
    ecs_entity_t entity = it->entities[i];

    bool added = false;
    AppWindowHandle *app_window_handle = ecs_get_mut(it->world, entity, AppWindowHandle, &added);

    ecs_add(it->world, it->entities[i], SdlWindow);

    const char *title = ecs_get_name(it->world, entity);
    if (!title) {
      title = "SDL2 window";
    }
      ...
      if (e.window.event == SDL_WINDOWEVENT_RESIZED ||
        ...
        for (int i = 0; i < it->count; i++) {
          if (SDL_GetWindowID(app_window_handle[i].value) == e.window.windowID) {
            ...
            SDL_GL_GetDrawableSize(app_window_handle[i].value, &actual_width, &actual_height);
...
static void SdlDestroyWindow(ecs_iter_t *it) {
  AppWindowHandle *window = ecs_term(it, AppWindowHandle, 1);
...
void SdlShutdown(ecs_world_t *world, void *ctx) {
  SDL_Quit();
  ecs_trace("SDL successfully shutdown.");
}

void SdlSystemImport(ecs_world_t *world) {
  ...
  ECS_SYSTEM(world, SdlProcessEvents, EcsPreUpdate, input.components.Input($), gui.components.AppWindow,
             gui.components.AppWindowHandle);
