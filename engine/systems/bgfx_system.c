...
#include "components/bgfx_components.h"
#include "bgfx_system.h"

static int SetPlatformData(SDL_Window *_window) {
  SDL_SysWMinfo wmi;
...
static void BgfxInitialize(ecs_iter_t *it) {
  Renderer *renderer = ecs_term(it, Renderer, 1);
  AppWindow *app_window = ecs_term(it, AppWindow, 2);
  AppWindowHandle *app_window_handle = ecs_term(it, AppWindowHandle, 3);
  ...
  for (int i = 0; i < it->count; i++) {
    ...
    if (!SetPlatformData(app_window_handle[i].value)) {
      ecs_err("Error: %s\n", SDL_GetError());
    }

    uint32_t debug = BGFX_DEBUG_PROFILER;
    uint32_t reset = BGFX_RESET_MAXANISOTROPY | BGFX_RESET_MSAA_X8 | BGFX_RESET_SRGB_BACKBUFFER;
...
static void OnAppWindowResized(ecs_iter_t *it) {
  AppWindow *app_window = ecs_term(it, AppWindow, 1);
  Bgfx *bgfx = ecs_term(it, Bgfx, 2);
...
void BgfxShutdown(ecs_world_t *world, void *ctx) {
  bgfx_shutdown();
  ecs_trace("BGFX successfully shutdown.");
}

void BgfxSystemImport(ecs_world_t *world) {
  ...
  ECS_SYSTEM(world, BgfxEndRender, EcsOnUpdate, [in] bgfx.components.Bgfx);
  ECS_OBSERVER(world, OnAppWindowResized,
               EcsOnSet, [in] gui.components.AppWindow, [in] bgfx.components.Bgfx);

  ecs_atfini(world, BgfxShutdown, NULL);
}