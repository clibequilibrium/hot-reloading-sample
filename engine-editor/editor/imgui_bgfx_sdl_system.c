...
// static float srgb_to_linear_float(float x) {
//   if (x <= 0.0f)
//     return 0.0f;
//   else if (x >= 1.0f)
//     return 1.0f;
...
static void ApplyImGuiDarkStyle() {
  ...
  //   style->Colors[i].x = srgb_to_linear_float(style->Colors[i].x);
  //   style->Colors[i].y = srgb_to_linear_float(style->Colors[i].y);
  //   style->Colors[i].z = srgb_to_linear_float(style->Colors[i].z);
  // }
}

static void ImguiInitialize(ecs_iter_t *it) {
  Input *input = ecs_term(it, Input, 1);
  AppWindowHandle *app_window_handle = ecs_term(it, AppWindowHandle, 2);

  for (int i = 0; i < it->count; i++) {
    if (!ecs_has(it->world, it->entities[i], SdlWindow)) {
      ecs_err("SdlWindow not found. Please make sure that SdlSystem was added.");
      return;
    }
    ...
    ecs_trace("ImGui %s initialized \n", version);
  }
}

static void ImguiUpdate(ecs_iter_t *it) {
  GuiContext *gui_context = ecs_term(it, GuiContext, 1);
  ...
  for (int i = 0; i < it->count; i++) {
    ...
    while (ecs_query_next(&gui_systems_iterator)) {
      GuiSystem *gui_systems = ecs_term(&gui_systems_iterator, GuiSystem, 1);
...
void ImguiBgfxSdlSystemImport(ecs_world_t *world) {
  ...
  ECS_OBSERVER(
      world, ImguiInitialize, EcsOnSet,
      input.components.Input($), [in] gui.components.AppWindowHandle, [in] gui.components.Renderer);

  ECS_SYSTEM(world, ImguiUpdate, OnRender, [in] gui.components.GuiContext);
  ecs_system_init(world, &(ecs_system_desc_t){.entity.entity = ImguiUpdate,
                                              .ctx = ecs_query_new(world, "GuiSystem")});
