...
static void DrawDockSpace(ecs_iter_t *it) {
  ...
  if (igBeginMenuBar()) {
    ...
    igText("%s | %s", ecs_get_name(it->world, it->entities[0]),
...
static void InitializeDockspace(ecs_iter_t *it) {
  GuiContext *gui_context = ecs_term(it, GuiContext, 1);

  for (int i = 0; i < it->count; i++) {
    ecs_entity_t gui_system = ecs_set_name(it->world, 0, "DockspaceSystem");
    ecs_set(it->world, gui_system, GuiSystem, {DrawDockSpace});
  }
}

void ImguiDockspaceSystemImport(ecs_world_t *world) {
  ECS_MODULE(world, ImguiDockspaceSystem);
  ECS_IMPORT(world, GuiComponents);

  ECS_OBSERVER(world, InitializeDockspace, EcsOnSet, [in] gui.components.GuiContext);
}